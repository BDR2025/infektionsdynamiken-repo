(function(){
  ...
  // ------------------------------
  // Manifest-backed file index (neu)
  // ------------------------------
  let TREE_ROOT = {};   // manifest.tree
  let MANIFEST  = null; // whole manifest
  let FILE_LIST = [];   // [{path, lc, node}]
  let FILE_MAP_LC = Object.create(null); // lcPath -> {path,node}

  function buildFileIndex(rootObj){
    FILE_LIST = [];
    FILE_MAP_LC = Object.create(null);
    (function walk(obj, prefix){
      ...
      for (const name of Object.keys(obj)){
        ...
        if (node && node.__type === 'file'){
          const info = { path: full, lc: full.toLowerCase(), node };
          FILE_LIST.push(info);
          FILE_MAP_LC[info.lc] = info;
  ...
  function findFileInfoByLoose(loosePath){
    if (!FILE_LIST.length) return null;
    const norm = normalizeLoosePath(loosePath).toLowerCase();

    // 1) exact
    if (FILE_MAP_LC[norm]) return FILE_MAP_LC[norm];

    // 2) try with allowed root
    if (ALLOWED){
      const withAllowed = (ALLOWED + '/' + norm).toLowerCase();
      if (FILE_MAP_LC[withAllowed]) return FILE_MAP_LC[withAllowed];
    }

    // 3) suffix match (robust gegen fehlenden Präfix wie 1_architecture/…)
    for (const it of FILE_LIST){
      if (it.lc.endsWith('/' + norm) || it.lc === norm) return it;
    }
    return null;
  }
  ...
  function linkifySafe(raw, baseDir){
    ...
    // 3) repo-relative file paths → bevorzugt CDN-Tab (wenn im Manifest bekannt), sonst interner Preview-Link
    const exts = LINKIFY_FILE_EXT.join('|');
    const re = new RegExp(`(^|[\\s])((?:\\.{0,2}\\/)?(?:[\\w\\-\\/]+)\\.(?:${exts}))`,'gi');

    out = out.replace(re, (all, lead, rel) => {
      // Versuche, den Pfad gegen das Manifest zu mappen (robust, auch bei /uid-e_v1/... ohne Präfix)
      const info = findFileInfoByLoose(rel);
      if (info && info.node && info.node.url){
        // -> externer CDN-Link in neuem Tab
        const url = info.node.url;
        return `${lead}<a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(rel)}</a>`;
      }

      // -> interner Preview-Link (wenn kein URL-Match gefunden)
      const target = info ? info.path : resolvePreviewTarget(rel, baseDir);
  ...
  async function boot(){
    try {
      ...
      // Build index once (für CDN-Tab-Links)
      buildFileIndex(TREE_ROOT);
